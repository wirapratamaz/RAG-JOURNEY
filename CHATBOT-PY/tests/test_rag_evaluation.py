...
# Add project root to PYTHONPATH before importing from src
...
contextual_precision = RAGASContextualPrecisionMetric(threshold=0.8)
contextual_recall = RAGASContextualRecallMetric(threshold=0.8)
answer_relevancy = RAGASAnswerRelevancyMetric(threshold=0.8)
faithfulness = RAGASFaithfulnessMetric(threshold=0.8)
...
@pytest.mark.parametrize("input_output_pair", input_output_pairs)
@patch('src.main.qa_chain.invoke')  # Mock the qa_chain.invoke method
def test_rag_metrics(mock_invoke, input_output_pair):
    # Extract input and expected output
    user_input = input_output_pair["input"]
    expected_output = input_output_pair["expected_output"]
    
    # Define the mock return value
    mock_invoke.return_value = {"answer": expected_output}
    
    # Generate response from the RAG application
    ...
    test_case = LLMTestCase(
        ...
        # retrieval_context=retrieval_context  # Include if applicable
    )
    
    # Assert the test case using DeepEval
